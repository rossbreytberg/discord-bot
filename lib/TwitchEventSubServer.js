      ...
      .on("end", () => {
        ...
        switch (request.headers["twitch-eventsub-message-type"]) {
          case "webhook_callback_verification":
            ...
            response.end();
            console.log(
              `Twitch EventSub server verified subscription "${subscription.type}" ${subscription.condition}`,
            );
            break;
          case "revocation":
            response.end();
            console.log(
              `Twitch EventSub server received revocation "${subscription.type}" ${subscription.condition}`,
            );
            break;
          case "notification":
            response.end();
            console.log(
              `Twitch EventSub server received notification "${subscription.type}" ${subscription.condition}`,
            );
