      ...
      .on("end", () => {
        ...
        switch (request.headers["twitch-eventsub-message-type"]) {
          case "webhook_callback_verification":
            ...
            response.end();
            console.log(`Twitch EventSub server verified subscription ${body}`);
            break;
          case "revocation":
            response.end();
            console.log(`Twitch EventSub server received revocation ${body}`);
            break;
          case "notification":
            response.end();
            console.log(`Twitch EventSub server received notification ${body}`);
            handleEvent(bodyParsed.subscription, bodyParsed.event);
            break;
        }
      });
  });
